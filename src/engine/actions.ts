 ...
 */
function getCriticalFailurePenalty(): number {
...
function executeDiplomacyAction(
  ...
  if (action.type === "propose_alliance") {
    ...
    if (baseSuccess) {
      ...
      if (existingRelation) {
        ...
        const duration = grade === "critical_success"
      ...
      if (grade === "critical_success") {
        ...
        changes.push(`【大成功】${targetClan.name}と固い同盟を締結！相手の信頼も厚い`);
      } else {
  ...
  if (action.type === "send_gift") {
    ...
    if (grade === "critical_success") {
      ...
      respectBoost = -5;
      leader.emotions.respect = Math.max(0, leader.emotions.respect + respectBoost);
  ...
  if (action.type === "threaten") {
    ...
    if (grade === "critical_success") {
      ...
      leader.emotions.discontent = Math.min(100, leader.emotions.discontent + 25);
...
function executeMilitaryAction(
  ...
  if (action.type === "attack") {
    ...
    // 戦闘計算（武勇で攻撃力ボーナス）
    const attackPower = soldierCount * (1 + action.riskTolerance * 0.3) * effectModifier * battleModifier;
    const defensePower =
    ...
    if (targetCastle.soldiers === 0 && remainingAttackers > 0) {
      ...
      return {
        success: true,
        grade: battleGrade === "critical_success" ? "critical_success" : "success",
        action,
      ...
      return {
        success: false,
        grade: battleGrade === "critical_failure" ? "critical_failure" : "failure",
        action,
...
function executeIntrigueAction(
  ...
  if (baseSuccess) {
    ...
    if (target) {
      ...
      switch (action.type) {
        case "bribe": {
          ...
          const originalClan = originalClanId ? state.clanCatalog.get(originalClanId) : null;
          ...
          if (!isLeader && target.emotions.loyalty <= 30) {
            ...
            if (grade === "critical_success") {
              ...
              changes.push(`${target.name}が寝返り！忠誠${target.emotions.loyalty + loyaltyDrop}→${target.emotions.loyalty}以下で決意`);
            }
          } else if (grade === "critical_success") {
            changes.push(`【大成功】${target.name}が完全に心変わり！（忠誠${target.emotions.loyalty}）`);
          } else {
            changes.push(`${target.name}を買収（忠誠-${loyaltyDrop}→${target.emotions.loyalty}）`);
          }
          break;
        }
        case "assassinate":
          ...
          if (target.clanId) {
            ...
            if (targetClan && targetClan.leaderId === target.id) {
              if (grade === "critical_success") {
                changes.push(`【大成功】${target.name}暗殺！闇に葬られた（当主死亡！）`);
              } else {
    ...
    if (grade === "critical_failure") {
      ...
      const target = state.bushoCatalog.get(action.targetId);
      if (target && target.clanId) {
        const targetClan = state.clanCatalog.get(target.clanId);
        if (targetClan) {
          const targetLeader = state.bushoCatalog.get(targetClan.leaderId);
          if (targetLeader) {
            targetLeader.emotions.discontent = Math.min(100, targetLeader.emotions.discontent + 20);
          }
          // 怨恨を記録
          const grudge: GrudgeEvent = {
      ...
      changes.push(`【大失敗】謀略が発覚！${target?.name || "対象"}の主君が激怒（怨恨発生）`);
    } else {
