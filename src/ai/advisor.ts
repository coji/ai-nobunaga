...
function getPersonalitySpeechStyle(personality: string[]): string {
  ...
  if (personality.includes("権威主義")) styles.push("格式を重んじ、威厳をもって");
...
// 議題のカテゴリを判定
type TopicCategory = "military" | "domestic" | "diplomacy" | "intrigue" | "general";

function categorizeTopc(topic: string): TopicCategory {
  const militaryKeywords = ["攻", "兵", "軍", "戦", "守", "城", "防衛", "侵攻"];
  const domesticKeywords = ["農", "商", "開発", "金", "財", "内政", "民", "発展"];
  const diplomacyKeywords = ["同盟", "外交", "交渉", "和平", "隣国", "敵国", "友好"];
  const intrigueKeywords = ["調略", "寝返", "流言", "買収", "謀略", "工作", "暗殺"];
...
// 議題に応じて代表者を選出（1-2人）
function selectRepresentatives(retainers: Busho[], category: TopicCategory): Busho[] {
...
export async function conductCouncilRound(
  ...
  const statements = await Promise.all(
    representatives.map(async (busho) => {
      ...
      // 武将ごとの性格・立場設定
      const characterSettings: Record<string, { rank: "veteran" | "middle" | "young"; style: string }> = {
        "柴田勝家": { rank: "veteran", style: "武骨な老将。主君には「〜にございます」、同僚には「〜じゃ」、若手には厳しく「〜せよ」" },
        "木下藤吉郎": { rank: "young", style: "人懐っこい出世頭。主君には「〜でございますれば」と丁寧、先輩には「〜ですなあ」、友には軽く「〜だよ」" },
        "丹羽長秀": { rank: "middle", style: "温厚な調整役。主君には「〜かと存じます」、誰にも「〜ではないかな」と穏やか" },
        "前田利家": { rank: "young", style: "熱血漢。主君には「〜でございます！」、先輩には「〜ですぞ」、友の秀吉には「〜だな！」" },
        "佐々成政": { rank: "young", style: "生真面目。主君には「〜と考えます」、誰にも堅い「〜であります」" },
        "池田恒興": { rank: "middle", style: "信長の乳兄弟で気さく。主君には「〜でしょう」、皆に「〜だろう」「まあまあ」" },
        "森可成": { rank: "veteran", style: "歴戦の猛将。主君には「〜にござる」、若手には「〜じゃ」と豪快" },
        "滝川一益": { rank: "middle", style: "冷静沈着。主君には「〜かと」、皆に「〜だな」と淡々" },
      };

      const setting = characterSettings[busho.name] || { rank: "middle", style: "丁寧に話す" };

      // 他の武将に話をふるかどうか（30%の確率）
      const shouldDelegateToOther = otherRetainerNames.length > 0 && Math.random() < 0.3;
      const delegateTarget = shouldDelegateToOther
        ? otherRetainerNames[Math.floor(Math.random() * otherRetainerNames.length)]
        : null;
...
【同席者との関係】
${otherNames.map((n: string) => {
  if (busho.name === "木下藤吉郎" && n === "柴田勝家") return `- ${n}: 対立関係（皮肉を言い合う仲）`;
  if (busho.name === "柴田勝家" && n === "木下藤吉郎") return `- ${n}: 対立関係（「猿め」と呼ぶ）`;
  if (busho.name === "前田利家" && n === "木下藤吉郎") return `- ${n}: 親友（気軽に話す）`;
  if (busho.name === "木下藤吉郎" && n === "前田利家") return `- ${n}: 親友（気軽に話す）`;
  if (busho.name === "佐々成政" && n === "柴田勝家") return `- ${n}: 尊敬する上官`;
  return `- ${n}: 同僚`;
}).join("\n")}
...
\`\`\``;
  ...
  for (const stmt of statements) {
    if (stmt.delegateTo) {
      const delegatedBusho = otherRetainers.find((r) => r.name === stmt.delegateTo);
      if (delegatedBusho) {
        // 簡単な返答を生成
        try {
          const response = await ai.models.generateContent({
...
export async function summarizeCouncilProposals(
  ...
  const enemyBushos = [...state.bushoCatalog.values()]
    ...
    .map((b) => {
      const bClan = state.clanCatalog.get(b.clanId!);
...
${enemyBushos || "  なし"}`);
  ...
  // カテゴリに応じた指示
  const categoryInstruction = category === "general"
    ? "議論で言及された内容に基づいて"
...
【重要】数字は必ずアラビア数字（1234）で書くこと。漢数字（一二三四）は使わない。
例: ×「二千百三十六の兵」 → ○「2136の兵」
...
export async function generateRetainerComments(
  ...
  const maxCommenters = grade === "critical_success" || grade === "critical_failure" ? 2 : 1;
...
\`\`\``;
    ...
    try {
      ...
      // 失敗時はデフォルトコメント
      const defaultComments: Record<ResultGrade, { supporter: string; opponent: string }> = {
        critical_success: { supporter: "見事にございます！", opponent: "これは驚きました" },
        success: { supporter: "よろしゅうございました", opponent: "ふむ…" },
        failure: { supporter: "残念でございます", opponent: "だから申したのに" },
        critical_failure: { supporter: "申し訳ございませぬ…", opponent: "予見通りでござる" },
      };
      results.push({
        bushoName: busho.name,
