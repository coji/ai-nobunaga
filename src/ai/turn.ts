...
- none: 何もしない`;
        ...
        switch (decision.action) {
          ...
          case "attack":
            ...
            if (attackTargets.length === 0) {
              ...
              const fromCastle = state.castleCatalog.get(decision.params.fromCastleId as string);
              const soldiers = (decision.params.soldierCount as number) ||
  ...
  return { actions: results, summary: results.length > 0 ? "行動完了" : "様子見" };
}
...
この指示を実行するために適切なツールを呼び出してください。`;
  ...
  if (!part || !("functionCall" in part) || !part.functionCall) {
    ...
    return {
      ...
      success: false,
      aiResponse:
        text || "コマンドを解釈できませんでした。もう少し具体的に指示してください。",
    };
  }
