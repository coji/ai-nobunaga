...
type Phase = "input" | "report" | "discussing" | "proposals" | "executing" | "result";

// プリセット議題
const PRESET_TOPICS = [
  { label: "状況報告", topic: "__REPORT__", isReport: true },
  { label: "今後の方針", topic: "今後、我が家はどのような方針で天下を目指すべきか" },
  { label: "軍備増強", topic: "兵を増やし軍備を整えるべきか、内政を優先すべきか" },
  { label: "隣国への対応", topic: "隣国にどう対処すべきか。攻めるか、同盟を結ぶか" },
  { label: "財政改善", topic: "金銭と兵糧をいかにして増やすべきか" },
  { label: "城の強化", topic: "どの城を重点的に強化すべきか" },
  { label: "調略", topic: "敵の武将を調略して寝返らせるか、流言で混乱させるか" },
  { label: "自由入力", topic: "" },
] as const;
...
export function CouncilScreen({
  ...
  const handleShowReport = async () => {
    ...
    // 状況をまとめるプロンプト
    const ownCastles = playerClan.castleIds.map(id => {
    ...
    const enemyInfo = [...state.clanCatalog.values()]
      ...
      .map(c => {
        ...
        const relation = state.diplomacyRelations.find(
          r => (r.clan1Id === playerClanId && r.clan2Id === c.id) ||
               (r.clan1Id === c.id && r.clan2Id === playerClanId)
        );
        const totalSoldiers = c.castleIds.reduce((sum, id) =>
          sum + (state.castleCatalog.get(id)?.soldiers || 0), 0);
...
【重要】数字はアラビア数字で書くこと。`;
  ...
  useInput(
    (input, key) => {
      ...
      if (!isNaN(num) && num >= 1 && num <= proposals.length) {
  ...
  useInput(
    (inputKey, key) => {
      ...
      // 数字キーでクイック選択
      const num = parseInt(inputKey);
      if (!isNaN(num) && num >= 1 && num <= PRESET_TOPICS.length) {
  ...
  const executeSelectedProposal = async () => {
    ...
    const grade: ResultGrade = result?.grade ?? (success ? "success" : "failure");
    ...
    <Box flexDirection="column">
      ...
      {phase === "report" && (
        <Box flexDirection="column" marginY={1}>
          <Text bold color="yellow">── 状況報告 ──</Text>
          {reportContent ? (
      ...
      {phase === "discussing" && (
        <Box flexDirection="column" marginY={1}>
          ...
          {statements.map((s, i) => {
            ...
            return (
              <Box key={i} marginLeft={isDelegate ? 3 : 1} flexDirection="column">
                <Box>
                  <Text>
                    ...
                    {s.targetName && (
                      <Text dimColor> →{s.targetName}に</Text>
                    )}
                    <Text>: </Text>
                  </Text>
                </Box>
                <Box marginLeft={2}>
                  <Text color={isDelegate ? "gray" : style.color}>「{s.statement}」</Text>
                </Box>
              </Box>
            );
          })}
        ...
        <Box flexDirection="column" marginY={1}>
          ...
          {showDiscussionLog ? (
            <Box flexDirection="column" marginBottom={1}>
              <Text bold color="yellow">── 議論ログ ──</Text>
              {statements.map((s, i) => {
                const style = getEmotionStyle(s.emotion);
                const isDelegate = s.isRepresentative === false;
                return (
                  <Box key={i} marginLeft={isDelegate ? 3 : 1}>
                    <Text>
                      {style.icon && `${style.icon} `}
                      {isDelegate && <Text dimColor>└ </Text>}
                      <Text color={isDelegate ? "gray" : style.color} bold>{s.bushoName}</Text>
      ...
      {phase === "result" && executionResult && (
        <Box flexDirection="column" marginY={1}>
          ...
          <Box borderStyle="round" paddingX={1} borderColor={
            executionResult.grade === "critical_success" ? "yellow" :
            executionResult.grade === "critical_failure" ? "magenta" :
            executionResult.success ? "green" : "red"
          }>
            <Text color={
              executionResult.grade === "critical_success" ? "yellow" :
              executionResult.grade === "critical_failure" ? "magenta" :
              ...
              {executionResult.grade === "critical_success" ? "★ 大成功！" :
               executionResult.grade === "critical_failure" ? "✗ 大失敗…" :
            ...
            <Box flexDirection="column" marginTop={1}>
              ...
              {executionResult.retainerComments.map((c, i) => (
                <Box key={i} marginLeft={1}>
                  <Text>
                    <Text bold color={
                      c.emotion === "praise" ? "green" :
                      c.emotion === "concern" ? "yellow" :
                      c.emotion === "angry" ? "red" :
                      c.emotion === "relieved" ? "cyan" : "white"
                    }>{c.bushoName}:</Text>{" "}
                    「{c.comment}」
                  </Text>
                </Box>
      ...
      {phase === "input" && !isCustomInput && (
        <Box flexDirection="column" marginTop={1}>
          <Text bold color="yellow">議題を選択:</Text>
          {PRESET_TOPICS.map((preset, i) => {
            ...
            return (
              <Box key={i} marginLeft={1}>
                <Text
                  color={isDisabled ? "gray" : i === selectedPreset ? "cyan" : "white"}
                ...
                {preset.topic && preset.topic !== "__REPORT__" && i === selectedPreset && !isDisabled && (
      ...
      {phase === "input" && isCustomInput && (
        <Box flexDirection="column" marginTop={1}>
          <Text bold color="yellow">自由入力:</Text>
          <Box marginTop={1}>
      ...
      <Box marginTop={1}>
        <Text dimColor>
          {phase === "input" && !isCustomInput && "[↑↓] 選択 [Enter] 決定 [1-7] クイック選択 [ESC] 戻る"}
          {phase === "input" && isCustomInput && "[Enter] 評定を開く [ESC] 戻る"}
