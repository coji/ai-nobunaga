...
import { factionList, diplomacyRelations } from "../src/data/diplomacy.js";
let hasError = false;
function error(msg) {
    console.error(`  ❌ ${msg}`);
    hasError = true;
...
for (const busho of bushoList) {
    ...
    if (!clanIds.has(busho.clanId)) {
        error(`武将 ${busho.name}(${busho.id}) の所属勢力 "${busho.clanId}" が存在しない`);
    }
    // factionIdの存在確認
    if (busho.factionId && !factionIds.has(busho.factionId)) {
        error(`武将 ${busho.name}(${busho.id}) の派閥 "${busho.factionId}" が存在しない`);
    }
}
// 2. 城データチェック
console.log("\n【城データ】");
...
for (const castle of castleList) {
    ...
    if (!clanIds.has(castle.ownerId)) {
        error(`城 ${castle.name}(${castle.id}) の所有者 "${castle.ownerId}" が存在しない`);
    }
    // castellanIdの存在確認
    if (castle.castellanId && !bushoIds.has(castle.castellanId)) {
        error(`城 ${castle.name}(${castle.id}) の城主 "${castle.castellanId}" が存在しない`);
    }
    // 隣接城の存在確認
...
for (const clan of clanList) {
    ...
    if (!bushoIds.has(clan.leaderId)) {
        error(`勢力 ${clan.name}(${clan.id}) の当主 "${clan.leaderId}" が存在しない`);
    }
    // castleIdsの存在確認
...
for (const clan of clanList) {
    for (const castleId of clan.castleIds) {
        ...
        if (castle && castle.ownerId !== clan.id) {
            error(`${clan.name} は ${castle.name} を所有しているが、城の ownerId は "${castle.ownerId}"`);
            ownershipErrors++;
        }
    }
}
...
for (const castle of castleList) {
    ...
    if (clan && !clan.castleIds.includes(castle.id)) {
        error(`${castle.name} の所有者は ${clan.name} だが、勢力の castleIds に含まれていない`);
        ownershipErrors++;
    }
}
...
for (const faction of factionList) {
    ...
    if (!clanIds.has(faction.clanId)) {
        error(`派閥 ${faction.name}(${faction.id}) の所属勢力 "${faction.clanId}" が存在しない`);
    }
    // memberIdsの存在確認
    for (const memberId of faction.memberIds) {
        if (!bushoIds.has(memberId)) {
            error(`派閥 ${faction.name}(${faction.id}) のメンバー "${memberId}" が存在しない`);
        }
    }
}
// 5. 外交関係チェック
...
for (const clan of clanList) {
    ...
    if (leader && leader.clanId !== clan.id) {
        error(`${clan.name} の当主 ${leader.name} の所属が "${leader.clanId}" になっている`);
        leaderErrors++;
    }
}
if (leaderErrors === 0) {
...
for (const castle of castleList) {
    if (castle.castellanId) {
        ...
        if (castellan && castellan.clanId !== castle.ownerId) {
            warn(`${castle.name} の城主 ${castellan.name} の所属(${castellan.clanId})が城の所有者(${castle.ownerId})と異なる`);
            castellanErrors++;
        }
    }
}
